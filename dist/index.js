...
async function generateFiles(projectDir, template, data) {
...
async function copyTemplateFiles(srcDir, destDir, data) {
...
async function processHandlebars(projectDir, data) {
...
function generatePackageJson(name, template, mcp) {
    const base = {
        name,
        version: '0.1.0',
    ...
    if (template === 'mcp-server') {
        const deps = { '@modelcontextprotocol/sdk': '^0.5.0' };
        if (mcp) {
    ...
    const deps = mcp ? { '@modelcontextprotocol/sdk': '^0.5.0' } : {};
    return JSON.stringify({ ...base, dependencies: deps, devDependencies }, null, 2);
}
